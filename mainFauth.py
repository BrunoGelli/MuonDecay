...
divisor = 3        #length in which each partial datafile will be splitted
eventsList = [] #list that stores event lists with 2500 points
#timeList = [] #list that stores the time when the events happen
...
pointsList = [] #List that stores the raw oscilloscope data
...
_ = ax.hist(differencesList,bins='auto', range=(0, 10), histtype='bar')
plt. title('Partial histogram with ' + str(len(differencesList)) + ' events')
...
while len(timeList)-initial <= necessarySamples:
    pointsList = scope.ch1.acquire_y_raw()
    eventsList.append(len(pointsList))
    ...
    if len(peaks) == 2:
        if peaks[0] > 50.0:
            if peaks[1] > 120.0:
                ...
                if microseconds > 0.1:
                    epochT1 = time.time()
                    timeList.append(epochT1)
                    differencesList.append(microseconds)
                    print('number of current samples: ', len(timeList)-initial)
                    if len(timeList) % divisor == 0:
                        filename = str(len(timeList))+'events_partialData.txt'
                        np.savetxt(filename, np.c_[differencesList, timeList], fmt='%.10e' , delimiter=", ")
                        np.savetxt(filename, differencesList, fmt='%.10e' , delimiter=", ")
...
np.savetxt("FullData.csv", np.c_[differencesList, timeList], fmt='%.10e', delimiter=", ")
